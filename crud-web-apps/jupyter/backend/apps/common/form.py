...
def add_ose_oauth_proxy(notebook, namespace):
    ...
    oauth_proxy = {
        ...
        "image": "registry.redhat.io/openshift4/ose-oauth-proxy:v4.9",
        "imagePullPolicy": "IfNotPresent",
        "args": [
            "--provider=openshift",
            "--https-address=:8443",
            "--http-address=",
            "--client-id=system:serviceaccount:%s:%s-notebook" % (
                namespace, notebook_name),
            "--client-secret-file=/var/run/secrets/kubernetes.io/serviceaccount/token",
            "--cookie-secret-file=/etc/proxy/config/session_secret",
            """--openshift-sar={"resource":"notebooks.kubeflow.org","verb":"get","name":"%s","namespace":"%s"}""" % (
                notebook_name, namespace),
        ...
        "volumeMounts": [
            {
                "name": "%s-notebook-proxy-config" % (notebook_name),
                "mountPath": "/etc/proxy/config"
            },
            {
                "name": "%s-notebook-tls-secret" % (notebook_name),
    ...
    oauth_proxy_volumes = [
        {
            ...
            "secret": {
                "defaultMode": 420,
                "secretName": "%s-notebook-proxy-config" % (notebook_name)
            }
        },
        {
            "name": "%s-notebook-tls-secret" % (notebook_name),
            "secret": {
                "defaultMode": 420,
                "secretName": "%s-notebook-tls" % (notebook_name)
            }
        }
    ]
